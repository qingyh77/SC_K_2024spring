...
method_char = 'CPD';  %DWCPD/ICPD  方法选择
...
file_path_expertisement = ['E:\hqy\Mat_R',num2str(RR),'_v',num2str(v),'_Rcp',num2str(Rcp)];%,'_trace1'];
file_path_cpd = [file_path_expertisement,'\Mat_rho',num2str(rho),'_R',num2str(RR),'_Rcp',num2str(Rcp),'_sinc2C\'];
%file_nameX = ['DWCPD_X_sigma',num2str(sigma),'.mat'];
file_nameX = ['Strue_',test_char,num2str(rho),'_R',num2str(RR),'.mat'];
...
file_nameA = ['S_A_',test_char,num2str(rho),'_R',num2str(RR),'.mat'];
file_nameB = ['S_B_',test_char,num2str(rho),'_R',num2str(RR),'.mat'];
...
for tt = 1:size(Xtrue_active,3)
    ...
    for ee = 1:epochs
        ...
        for rr = 1:1
            if diff_on == 0
                ...
                C_icpd = load([file_path_cpd,'S_C_rho',num2str(rho),'_R',num2str(RR),'_',num2str(rr)]);
    ...
    %             ICPD_active = 31-1*ceil(1.5*2/rho);
                ...
                Xhat_nmfcpd_lstm = Shat_lstm_rr;
            else
                C_temp = load([file_path_net,file_nameC_lstm,'_',num2str(rr),'_diff.mat']);
                C_temp = C_temp.C_pred;
                C_temp = squeeze(C_temp(:,ee,:));

                C_icpd = load([file_path_cpd,'S_C_rho',num2str(rho),'_R',num2str(RR),'_',num2str(rr)]);
                C_icpd = C_icpd.C_icpd;
                C_tc = C_icpd(batch_size+time_step+1:end,:);

                C_temp = cumsum([C_tc(1,:);C_temp]);
                NMSE_c(tt,rr) = frob(C_tc(tt,:)- squeeze(C_temp(tt,:))').^2/frob(C_tc(tt,:)).^2; 
    %             ICPD_active = 31-1*ceil(1.5*2/rho);
                %C_temp = C_temp(ICPD_active:end,:,:);
        ...
        NMSE_lstm_tt(tt,ee) = frob(Xtrue_tt_tens - Shat_lstm_rr).^2/frob(Xtrue_tt_tens).^2;
    end
    if tt == 50 || tt==500 || tt==900
        ...
        tiledlayout(2,1);
        nexttile;
        h1 = contourf(10*log10(Xtrue_tt_tens),100,'linecolor','None');
        hold on;
...
%         colorbar();
        ...
        h2 = contourf(10*log10(Xhat_nmfcpd_lstm),100,'linecolor','None');
        hold on;
        ...
        set(gca,'xtick',[],'xticklabel',[])
        set(gca,'ytick',[],'yticklabel',[])
        title(['$\hat{S}$',',t=',num2str(tt),',NMSE=',num2str(NMSE_lstm_tt(tt,end))],'Interpreter','latex','FontName','Times New Roman');
...
%% 张量补全误差
Ci_R_T = load([file_path_cpd,'S_C_rho',num2str(rho),'_R',num2str(RR),'.mat']);
Ci_R_T = Ci_R_T.Ci_R_T;

Ai_R_T = load([file_path_cpd,'S_A_rho',num2str(rho),'_R',num2str(RR),'.mat']);
Ai_R_T = Ai_R_T.Ai_R_T;

Bi_R_T = load([file_path_cpd,'S_B_rho',num2str(rho),'_R',num2str(RR),'.mat']);
Bi_R_T = Bi_R_T.Bi_R_T;

file_nameC_TC = ['S_C_rho',num2str(rho),'_R',num2str(RR)];
...
%             ICPD_active = 31-1*ceil(1.5*2/rho);
    ...
    NMSE_TC_tt(tt) = frob(Xtrue_tt_tens - Shat_TC_rr).^2/frob(Xtrue_tt_tens).^2;
end
...
%% 画图
figure();
plot(NMSE_lstm_tt(:,end),'linewidth',1.5,'DisplayName', ...
                ['CPD-LSTM,$\rho$=',num2str(rho),',epochs=',num2str(10)])
hold on;
plot(NMSE_TC_tt,'linewidth',1.5,'DisplayName', ...
                ['CPD,$\rho$=',num2str(rho)])
xlabel('T');
ylabel('NMSE');
set(gca,'FontName','Times new roman');
grid on;
title(['$\rho$=',num2str(rho),',epochs=',num2str(10)],'Interpreter','latex')
set(gca,'GridLineStyle',':');
legend('Interpreter','latex');
xlim([1,size(NMSE_time{1},1)]);
set(gca,'Yscale','log');
ylim([1e-2,2])
...
if diff_on == 0
    ...
    save_file_name = ['NMSE_',method_char,'_rho',num2str(rho),'_R',num2str(RR),'_timestep=',num2str(time_step),'_diff.mat'];
end
