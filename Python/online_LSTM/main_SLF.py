...
for rr in range(0,len(rho_set)):
    ...
    for ii in range(1,2):
        ...
        experiment = 'Mat_R'+str(R_num) + '_v' + str(v) + '_Rcp'+str(Rcp)
        #file_path = 'E:\\hqy\\Sigma_net\\采样位置固定\\'
        #file_path = 'F:\\hqy\\Mat_R8_SNR20_sinc2C\\'
        file_path = 'E:\\hqy\\'+experiment + '\\Mat_rho'+str(rho)+'_R%d' %R_num+'_Rcp'+str(Rcp)+'_sinc2C\\'
        #file_path = 'F:\\黄清扬\\Sigma_cpd\\NET\\'
        #file_name = 'S_C_rho'+str(rho)+'_R%d.mat' %input_index
        file_name = 'S_C_rho' + str(rho) + '_' + 'R%d' % R_num + '_%d.mat' % input_index
        ...
        for tt in range(ts_start,Time_len):
            print("----采样率{}，辐射源{}，第{}时刻开始----".format(rho,input_index,tt))
        ...
        model_savename = file_path+'NEToutput\\' + 'Net_timestep20' + '_rho' + str(rho) + '_input%d' % input_index + '.pth'
        torch.save(model,model_savename)
        C_save2 = C_save2.detach().numpy()
        filename3 = 'CPD_LSTM_rho'+str(rho) + '_timestep=20_online_%d.mat'%input_index
