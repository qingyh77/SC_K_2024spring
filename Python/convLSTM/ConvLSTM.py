...
class ConvLSTM_baseline(nn.Module):
    def __init__(self,config=None):
        super().__init__()
        self.conv_lstm = ConvLSTM(in_channels=config.in_channels,
                                  out_channels=config.out_channels,
                                  kernel_size=config.kernel_size,
                                  num_layers=config.num_layers,
                                  batch_first=config.batch_first,
                                  bias=config.bias)
        self.linear = nn.Linear( in_features=config.out_channels,out_features=1)

    def forward(self,x,config):
        seq_layer_output, layer_output = self.conv_lstm(x)
        seq_output_tensor = seq_layer_output[-1].view(-1,config.out_channels)
        prediction = self.linear(seq_output_tensor)
        prediction = prediction.view(config.batch_size,config.time_step,config.in_channels,config.height,config.width)
        return prediction
