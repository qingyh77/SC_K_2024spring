...
snr = 20
v=0.01
rho_set = [0.12,0.14,0.16,0.18,0.2]
for rr in range(0,len(rho_set)):
    rho = rho_set[rr]

    # file_path = 'E:\\hqy\\Sigma_net\\采样位置固定\\'
    file_path_1 = 'E:\\黄清扬\\SpectrumPrediction_2024\\MATLABoutput\\Mat_R%d' % sourceNum + '_v'+ str(v) + '_SNR%d' %snr
    file_path_2 = '_张量补全失败\\Mat_rho'+str(rho)+'_R%d'% sourceNum + '_SNR%d' %snr +'_sinc2C\\'
    ...
    def train_layers(layers_list=None, layer=None, epoch=None, validate=True, weight_decay_list=None, Train_Data=None):
        ...
        for epoch_index in range(epoch):
            ...
            for batch_index, i in enumerate(range(0, Train_Data.size(0), batch_size)):
                ...
                ## 结果打印
                if (batch_index + 1) % math.ceil(len(Train_Data) / 10 / 2) == 0 and (
                        (epoch_index + 1) == epoch / epoch or (epoch_index + 1) == epoch):  # (batch_index+1) == 1 or
    ...
    def train_whole(model=None, epoch=50, Train_Data=None):
        ...
        for epoch_index in range(epoch):
            ...
            for batch_index, i in enumerate(range(0, Train_Data.size(0), batch_size)):
                ...
                if (batch_index + 1) % math.ceil(len(Train_Data) / 10 / 2) == 0 and (
                        (epoch_index + 1) == epoch / epoch or (epoch_index + 1) == epoch):
    ...
    for tt in range(t_start, t_end):
        for ii in range(I):
            ...
            for level in range(num_layers):
                train_layers(layers_list=layers_list, layer=level, epoch=AE_train_epochs, validate=True,
                             weight_decay_list=L2_weight, Train_Data=data_seq)

            SAEL_model = SAEL(layers_list=layers_list, input_size=32, hidden_layer_size=100, output_size=64)
            _, C_pred = train_whole(SAEL_model, 10, data_seq)
